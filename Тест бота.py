...
def help_cmd(message):
    ...
    bot.send_message(
        ...
        "- Бот покажет погоду на выбранный день и даст полезные советы."
    )
...
def get_weather(bot, chat_id, city, date):
    ...
    Логика:
        ...
        3. Форматирование и отправка сообщения с погодой и рекомендациями
    """
    ...
    try:
        ...
        description = best['weather'][0]['description']  # Описание (в нижнем регистре для рекомендаций)
        desc_display = description.capitalize()          # Описание для отображения (с заглавной буквы)
        ...
        # Генерация персонализированной рекомендации на основе погодных условий
        recommendation = generate_recommendation(temp, description, hum, wind)
